...
void LoginAcceptObject(CObject* pObj)
{
    ...
    // 2. PACKET_SC_CREATE_OTHER_CHARACTER 에 연결된 세션의 정보를 담아 브로드캐스트
    //=====================================================================================================================================
    SC_CREATE_OTHER_CHARACTER_FOR_All(pSession, pPlayer->m_ID, pPlayer->GetDirection(), posX, posY, pPlayer->GetHp());
    ...
    for (const auto& mapIndex : ObjectManager.GetObjectMap())
    {
        CPlayer* player = static_cast<CPlayer*>(mapIndex.second);

        player->getPosition(posX, posY);
        SC_CREATE_OTHER_CHARACTER_FOR_SINGLE(pSession, player->m_ID, player->GetDirection(), posX, posY, player->GetHp());

        // 움직이고 있는 상황이라면
        if (player->isBitSet(FLAG_MOVING))
        {
            SC_MOVE_START_FOR_SINGLE(pSession, player->m_ID, player->GetDirection(), posX, posY);
        }
    }
}